...
for (const quality of ['mobile', 'desktop', 'cinematic']) {
  test(`actual-note corona joins the direct disk at the reproduced inclined seam: ${quality}`, async ({ browser, baseURL }, testInfo) => {
    ...
    try {
      ...
      expect(actual.noteStars).toBe('1675'); expect(actual.diskSourceStars).toBe('1675');
