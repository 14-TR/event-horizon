...
test('Locate flushes residual orbit damping and reduced-motion/pausing settles the live target immediately', () => {
  const view = makeView(390, 844, false), id = 'n001614';
...
test('renderer failure and disposal stop navigation, clear context and dispose the overlay once', () => {
  ...
  try {
    view.selectNode('n001614');
    view.setConnectionContext('n001614');
    view.locateNode('n001614');
    ...
    assert.equal(view.locateNode('n001614'), false);
    assert.deepEqual(view.setConnectionContext('n001614'), { id: null, neighborId: null, rendered: 0, displayed: 0, total: 0 });
...
test('Locate uses edge safe rectangles for selected reticles but rejects a zero-sized viewport', () => {
  const view = makeView(320, 568), node = view.layout.nodes.find(n => n.id === 'n000002');
...
test('paused Locate frames the actual animated destination inside mobile and desktop safe rectangles', () => {
  ...
  for (const [width, height, rect] of [
    ...
    view.updateInfall(8);
    const node = view.layout.nodes.find(node => node.id === 'n001614');
...
test('selected context shows only bounded genuine incident edges including the requested cross-sector neighbor', () => {
  ...
  const id = 'n001614', neighborId = 'n000002';
  ...
  assert.equal(result.rendered, 12, 'the preview stays a quiet context even for an 814-neighbor hub');
  ...
  assert.equal(view.host.dataset.connectionRendered, '12');
  assert.equal(view.visibleEdgeCount, 12, 'reported visible edges mean the actual overlay draw budget');
  const hits = [];
  ...
  assert.equal(view.host.dataset.noteStars, '1675');
});
...
test('manual navigation cancellation relinquishes live tracking and selection/sector changes clear stale context', () => {
  ...
  view.locateNode(id);
  view.selectNode('n000002');
  assert.equal(view.flight, null);
  assert.equal(view.connections.line.visible, false);
  assert.equal(view.host.dataset.connectionTotal, '0');
  view.setConnectionContext('n000002');
  view.locateNode('n000002');
  ...
  assert.equal(view.locateNode('n000002'), false, 'a hidden sector is not silently revealed');
  assert.equal(view.locateNode(' n001614'), false, 'identifiers are never repaired');
  view.locateNode(id);
  view.setNavigationMode('fly');
  assert.equal(view.locating, null);
});
