...
The production base is **`/event-horizon/`**, suitable for a GitHub Pages project site. A separately authorized **daily local anonymous topology workflow** can supply changing data; the application itself never accesses a vault. Exporter code, scheduling, credentials, source mappings and operational receipts stay outside this public repository. Each candidate must pass the closed-world publication whitelist, tests, build and review before publication; this compatibility support neither authorizes publication nor enables a schedule. Browser tests use synthetic network-intercepted fixtures plus read-only tests of the real `public/graph.json`. They never overwrite that file. Screenshots and coverage receipts are written to ignored `test-results/`.
...
Each actual note also supplies a finite, flow-aligned **light volume**, centered at its live position with its current color and size. A compact ellipsoidal emission kernel has real radial and vertical support, so its light does not vanish into a zero-area sheet at exactly edge-on. A curved, domain-warped wisp profile modulates only that source's bounded footprint. Nearby source volumes share a continuous disk-space light grade, so their overlapping detail reinforces rather than averaging into a smooth torus. That grade rotates on the notes' simulation clock; individual notes still follow their unchanged inward trajectories through it. See [actual-note flow](docs/NOTE-FLOW.md) for the approximation and GPU regressions. These are not added gas particles, orbital histories, graph nodes or pickable objects. Hiding a source hides its volume. The direct view and HDR capture use the same geometry and emission kernel. The kernel's optical column is integrated analytically; turbulence and curvature are evaluated at the ray's closest point rather than marched through a full density field. This is an artistic, column-filtered volume approximation, not a physical point-spread function or full radiative transfer.
